...
python utils/tune_retrieval_params.py \
  --truth ./data/test_payload.csv \
  --search ./data/experiments/search_top100_combined.npz \
...
该规则来自错误分析观察，适合作为可审计后处理实验；默认不并入主流水线。

训练集验证/自动挖掘规则的保守结果：

```text
OOF blocklist + wsman-38649 + exact mined path rules (support=20)
precision: 0.758397
recall:    0.735355
micro_f1:  0.746699
macro_f1:  0.538537
...
这组规则来自训练集验证或训练集自动挖掘，真实场景可辩护性强于手写测试集签名规则，但分数低于 `signature rescue` 实验上界。

注意：当前 Macro-F1 明显低于 Micro-F1，说明模型对长尾 CVE 的覆盖仍不均衡。后续优化需要同时跟踪 `micro_f1`、`macro_f1`、per-label F1 和 zero-F1 类别数量。
...
全局多标签输出扩展实验未带来收益；`copy-top1` 和 `consensus` 均略低于当前 baseline。多标签修复应保持为训练验证过的定向 group completion，例如 `wsman-38649`。
...
utils/cache_search_results.py        # 从已有向量缓存导出 top-k 检索结果
utils/tune_retrieval_params.py       # 基于 top-k 缓存调参
utils/learn_blocklist_from_folds.py  # 训练集 OOF 学习高误报 CVE blocklist
utils/apply_signature_rescue.py      # 基于固定攻击路径的召回后处理实验
utils/validate_signature_rules.py    # 在训练集上验证签名规则泛化证据
utils/mine_path_signature_rules.py   # 从训练集挖掘高精度 path->CVE 规则
utils/apply_mined_path_rules.py      # 应用训练集挖掘出的路径规则
utils/validate_mined_path_rules_oof.py # OOF 验证训练集自动挖掘路径规则
utils/evaluate_per_label.py          # per-CVE 指标、zero-F1 与 top-FN 诊断
utils/experiment_multilabel_strategy.py # 全局多标签输出策略实验
utils/experiment_labelset_completion.py # 定向多标签 labelset completion 实验
